...
//     let entries = Array.isArray(entriesProp) ? entriesProp : [];
//
...
//             liveEntries: [],
//             backupEntries: [],
//             tableData: [{id: 'section-live', isSection: 'live'}],
//             sortableIds: [],
//         };
//     }
//
//     // 2. Tách visible / invisible
//     const visibleEntries = [...entries].filter(e => e.visible === true);
//
//     // 3. Live: tối đa liveCount visible entries
//     const liveEntries = visibleEntries.slice(0, liveCount);
//
//     // 4. Backup: visible thừa + tất cả invisible
//     const visibleIds = new Set(liveEntries.map(e => e.id));
//     const backupEntries = [...entries].filter(e => !visibleIds.has(e.id));
//
//
//     // 5. Gán lại position
//     const finalLive = liveEntries.map((item, idx) => ({
//         ...item,
//         position: idx + 1,
//     }));
//
...
//     ] : [];
//
//     // 7. Sortable IDs - RẤT QUAN TRỌNG cho drag & drop
//     entries = [...finalLive, ...finalBackup];
...
//         // const finalEntries = processLiveAndBackup(newSorted, liveCount);
...
//     // };
//
//     // Toggle visibility của một entry + tự động sắp xếp lại Live / Backup
//     const handleToggleVisible = (id: string) => {
//         const newEntries = entries.map((entry) => {
//             if (entry.id === id) {
